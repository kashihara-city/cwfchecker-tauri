...
- **30秒の固定許可ウィンドウとして対応済み。**
- 元の指摘にある「`auth_count > 0`を検出するまでフラグが閉じない」という説明は誤りで、
  設定先オリジンから有効なページレポートを受信した時点で、認証成功の有無にかかわらず
  外部HTTPS許可は終了していた。
- 対応後は、最初に設定先以外のHTTPSへ遷移した時点から30秒間だけ、SAML IdP等への
  外部HTTPS遷移を許可する。遷移のたびに期限は延長しない。
- 最初のポートレットPOSTでは許可を閉じず、一度外部へ出た後に設定先オリジンへ戻った時点で
  即座に許可を終了する。30秒を過ぎた外部HTTPS遷移も拒否する。
- ダウンロード開始時のトップレベルページが設定先オリジンでない場合は、
  ダウンロードと関連付けアプリでの自動起動を拒否する。
